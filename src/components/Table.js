...
const Table = ({ news }) => {
	...
	const list = news.map((data, index) => {
		...
		const intro =
			data.content !== null
				? data.content.replace(/[\\+[0-9]{0,10} chars]$/gi, "")
				: "Please visit page to see content";
		const sourceName = data.source.name.match(/^\S/);
		const date = data.publishedAt
			.replace(/(T\d{0,2}.\d{0,2}.\d{0,2}.\d{0,100}Z)$/, "")
			.split("-")
			.reverse()
			.join("-");
		...
		if (data.urlToImage === null) {
			data.urlToImage =
				"https://images.unsplash.com/photo-1556740714-a8395b3bf30f?ixlib=rb-1.2.1&q=80&fm=jpg&crop=entropy&cs=tinysrgb&w=1080&fit=max&ixid=eyJhcHBfaWQiOjYxNjQ5fQ";
		}
		return (
